...
import {
    ...
    Fingerprint, Megaphone, MessageCircle, MousePointer2
} from 'lucide-react';
...
export const AmplifyLogo3D: React.FC<{ className?: string }> = ({ className }) => {
    ...
    return (
        <div className={`relative ${className}`}>
            ...
            {mounted ? (
                <Spline scene="https://prod.spline.design/DIJJCbomtTdoIowC/scene.splinecode" />
            ) : (
                <div className="w-full h-full rounded-md bg-gradient-to-br from-indigo-100 to-purple-100" />
            )}
        </div>
    )
}
...
export const LogoMarquee: React.FC = () => {
    return (
        <div className="w-full overflow-hidden opacity-80 hover:opacity-100 transition-opacity duration-300 mask-linear-fade">
            <div className="flex animate-marquee whitespace-nowrap hover:[animation-play-state:paused] items-center py-4">
                {[...LOGOS, ...LOGOS, ...LOGOS].map((logo, i) => (
                    <div key={i} className="group mx-8 md:mx-12 flex items-center gap-3" tabIndex={0}>
                        {/* Abstract Logo Icon */}
                        <div className="w-6 h-6 rounded bg-white/10 group-hover:bg-white/20 border border-white/10 group-hover:border-white/30 backdrop-blur-sm flex items-center justify-center transition-all duration-300">
                            <div className="w-3 h-3 rounded-full bg-current opacity-50" />
                        </div>
                        <span className="text-base md:text-lg font-display font-medium text-gray-400 group-hover:text-white transition-colors">{logo}</span>
                    </div>
                ))}
            </div>
        </div>
        ...
        <div className="relative w-full max-w-[1000px] mx-auto mt-10 mb-20 z-20 perspective-1000">
           ...
           ========================================== */}
                                                    ...
                                                    <div key={i} className="border-r border-b border-gray-50 p-1 relative hover:bg-gray-50 transition-colors">
                                                        ...
                                                        {[3, 8, 12, 15, 18, 22, 25].includes(i) && (
                                                            <motion.div
                                                                ...
                                                                className={`mt-1 w-full p-1.5 rounded-md border shadow-sm cursor-pointer hover:shadow-md transition-all flex flex-col gap-1 ${i % 3 === 0 ? 'bg-purple-50 border-purple-100' :
                                        ...
                                        <div className="space-y-2.5">
                                            {[
                                                ...
                                                >
                                                    ...
                                                    <div className={`text-[9px] font-bold px-2 py-1 rounded-full border ${item.status === 'Published' ? 'bg-green-50 text-green-600 border-green-100' :
            ...
            <motion.section
                ...
                className="relative mt-16 md:mt-24 px-4 pb-12"
            >
                <h2
                    id="trusted-heading"
                    className="text-center text-xs md:text-sm font-bold text-gray-500 uppercase tracking-[0.2em] mb-8"
                >
                ...
                <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 max-w-5xl mx-auto">
                    {[
                        { label: "SOC 2 & GDPR", icon: ShieldCheck, color: "text-[#00d084]", bg: "bg-[#00d084]/10", border: "border-[#00d084]/20" },
                        { label: "12,000+ users", icon: Users, color: "text-[#38BDF8]", bg: "bg-[#38BDF8]/10", border: "border-[#38BDF8]/20" },
                        { label: "99.98% uptime", icon: Clock, color: "text-[#9d4edd]", bg: "bg-[#9d4edd]/10", border: "border-[#9d4edd]/20" },
                        { label: "+324% ROI uplift", icon: TrendingUp, color: "text-[#ff6b35]", bg: "bg-[#ff6b35]/10", border: "border-[#ff6b35]/20" }
                    ].map((item, i) => (
                        <div
                            key={i}
                            className="group relative flex items-center gap-4 p-4 rounded-xl border border-white/5 bg-white/[0.02] hover:bg-white/[0.05] backdrop-blur-sm transition-all duration-300 hover:scale-[1.02] hover:border-white/10"
                        >
                            <div className={`w-10 h-10 rounded-lg ${item.bg} ${item.border} border flex items-center justify-center ${item.color} group-hover:scale-110 transition-transform`}>
                                <item.icon size={18} />
                            </div>
                            <div>
                                <div className="text-white font-semibold text-sm tracking-wide">{item.label}</div>
                                {/* Subtle glow line */}
                                <div className={`h-0.5 w-0 group-hover:w-full transition-all duration-500 ease-out bg-current ${item.color} opacity-40 mt-1 rounded-full`} />
                            </div>
                        </div>
                    ))}
                </div>
